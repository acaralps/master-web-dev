...
<body>
    ...
    <?php
        ...
        $arrayImagen = array();
        $sql = "SELECT * FROM private_messages WHERE emitter='$id'";
        ...
        $arrayNombre = array();
        $sql="SELECT users.name,users.surname,users.id,private_messages.emitter FROM users INNER JOIN private_messages ON users.id=private_messages.receiver WHERE private_messages.emitter=$id";
        $result2 = $conn->query($sql);
        if ($result2->num_rows > 0) {
            while ($row = mysqli_fetch_array($result2)) {
            array_push($arrayNombre,$row["name"]." ".$row["surname"]);
            }
        }
    ?>
    <script>
